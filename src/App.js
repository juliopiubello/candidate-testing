...
const App = () => {
  ...
  const sendIntroductionAgain = (fn, val, retries = 10, time = 2000) => {
    return fn()
      .then((result) => {
        if (result) {
          return val;
          ...
          return delay(time).then(() => {
            if (retries > 1) {
              return sendIntroductionAgain(fn, val, retries - 1, time);
            } else {
      ...
      .catch((err) => {
        return delay(time).then(() => {
          retries > 1
            ? sendIntroductionAgain(fn, val, retries - 1, time)
            : console.log(err);
        });
      });
  };

  const checkStatusAgain  = (fn, val, retries = 10, time = 2000) => {
    return fn(val)
      .then((result) => {
        if (result.status === "online") return result;
          ...
          return delay(time).then(() =>
            retries > 1
              ? checkStatusAgain(fn, val, retries - 1, time)
      ...
      .catch((err) => {
        return delay(time).then(() =>
          retries > 1 ? checkStatusAgain(fn, val, retries - 1, time) : false
        );
      });
  };

  const AllToArray = (arrayCheckStatus) => {
    Promise.allSettled(arrayCheckStatus).then((results) =>
        ...
        if (result.value.status === "online") {
          sendIntroduction(result.value.id).then((res) => {
            if (res) {
              ...
              sendIntroductionAgain(sendIntroduction, result.value.id).then(
                (val) => {
                  if (val) {
                    setUsers((oldUsers) => [...oldUsers, result.value.id]);
                  }
          ...
          checkStatusAgain(checkStatus, result.value.id).then(
            (resultFromStatus) => {
              if (resultFromStatus) {
